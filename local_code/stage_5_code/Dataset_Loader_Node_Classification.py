...
class Dataset_Loader(dataset):
    ...
    def load(self):
        ...
        val_size = 300
        training_size, testing_size = None, None
        ...
        if not training_size or not testing_size:
            raise ValueError("dataset_name invalid")

        # idx_all = random.sample(range(len(labels)), val_size + training_size + testing_size)
        idx_all = np.random.choice(np.arange(0, len(labels)), size=val_size + training_size + testing_size, replace=False)

        print(idx_all[:10])

        idx_train = idx_all[:training_size]
        idx_test = idx_all[training_size:training_size + testing_size]
        idx_val = idx_all[training_size + testing_size:]
